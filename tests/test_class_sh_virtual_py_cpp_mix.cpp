...
struct BaseVirtualOverrider : Base, py::trampoline_self_life_support<BaseVirtualOverrider> {
    using Base::Base;

    int get() const override { PYBIND11_OVERRIDE(int, Base, get); }
};

struct CppDerivedVirtualOverrider : CppDerived, py::trampoline_self_life_support<CppDerivedVirtualOverrider> {
